...
class DataLoader:
    ...
    def _load(self):
        ...
        lectures_types = {"lecture_id": "int16", "tag": "int16", "part": "int8", "type_of": "object"}
    ...
    def _read_csv(self, filename, filetypes):
        return pd.read_csv(
            os.path.join(self.path, filename), usecols=filetypes.keys(), dtype=filetypes, low_memory=False
        )
...
def preprocess_questions(questions):
    ...
    questions.rename(
        columns={"question_id": "content_id", "part": "question_part", "tags": "question_tags"}, inplace=True
    )
    return questions
...
def preprocess_lectures(lectures):
    ...
    lectures["type_of"] = lectures["type_of"].map({"concept": 0, "solving question": 1, "intention": 2, "starter": 2})
