...
class QuestionsEmbedder(TransformerMixin):
    ...
    def fit(self, X, y=None):
        ...
        if get_cache_manager().exists(cache_id):
            ...
            model = Word2Vec(sentences=sentences, size=self.size, window=self.window, min_count=self.min_count, sg=self.sg, iter=self.iter, workers=self.workers)
        ...
        cache_id = 'questions_kmean_{}_{}_{}_sentences'.format(
            self.n_clusters, self.kmean_n_init, len(sentences)
        )
...
class AnswersCorrectnessEmbedder:
    ...
    def _fit(self, X, fold=None):
        ...
        if get_cache_manager().exists(cache_id):
            ...
            model = Word2Vec(sentences=sentences, size=self.size, window=self.window, min_count=self.min_count, iter=self.iter, sg=self.sg, workers=self.workers)
            get_cache_manager().save(model, cache_id)
    ...
    def _compute_similarity(self, X, model, lag):
        ...
        for r in range(len(user_id)):
            ...
            if len(user_content_id) >= lag:
                sim_correct = self._get_similarity(model, user_content_id[-lag], content_id[r], user_answered_correctly[-lag], 1)
