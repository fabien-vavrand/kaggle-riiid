...
class SessionFeaturer:
    ...
    def _fit(self, X, y=None):
        ...
        # Build context
        self.context = X[["user_id", "timestamp", "task_container_id", "session_id", "session_start"]].drop_duplicates(
            "user_id", keep="last"
        )
        self.context.rename(
            columns={"timestamp": "prior_timestamp", "task_container_id": "prior_task_container_id"}, inplace=True
        )
        questions_context = X[X["content_type_id"] == 0][["user_id", "content_time"]].drop_duplicates(
            "user_id", keep="last"
        )
    ...
    def _compute_time_features(self, X):
        ...
        tasks = (
            X[X["content_type_id"] == 0]
            .groupby(["user_id", "session_id", "task_container_id"], sort=False)
            .agg({"content_time": [np.sum, "count"]})
        )
        ...
        tasks = (
            X[X["content_type_id"] == 0]
            .groupby(["user_id", "task_container_id"], sort=False)["answered_correctly"]
            .mean()
            .reset_index(name="prior_answered_correctly")
        )
        ...
        for r in range(users.shape[0]):
            ...
            try:
                ...
                if ts - context["prior_timestamp"] >= self.hours_between_sessions * 60 * 60 * 1000:
                    context[
                        "prior_timestamp"
                    ] = np.nan  # So we are not able to compute content_time for the first element of a session
...
class LecturesFeaturer:
    ...
    def _fit(self, X):
        ...
        last_lecture_features = [
            "tasks_since_last_lecture",
...
class QuestionsFeaturer:
    ...
    def fit(self, X, y=None):
        ...
        questions = (
            X[["content_id", "content_time"]].dropna().groupby("content_id")["content_time"].agg(["median", "count"])
        )
        ...
        self.questions["mean_content_time"] = (
            weight * self.questions["mean_content_time"] + (1 - weight) * self.content_time
        )
    ...
    def _get_n_lectures_on_question_part(X):
        ...
        for i, r in enumerate(
            X[["question_part"] + [f"n_lectures_part{p}" for p in range(1, 8)]].itertuples(index=False, name=None)
        ):
