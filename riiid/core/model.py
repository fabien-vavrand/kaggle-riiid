...
from riiid.core.transformers import (
    ColumnsSelector,
    MemoryUsageLogger,
    RatioTransformer,
    WeightedAnswerTransformer,
    TypesTransformer,
    DensityTransformer,
)
...
class RiiidModel:
    ...
    def make_global_pipeline(self, cv):
        ...
        self.pipeline = make_pipeline(
            ...
            ScoreEncoder(
                ["tasks_bucket_3", "content_id"],
                cv=cv,
                parent_prior="content_id_encoded",
                updatable=True,
                **self.params["score_encoder_2"]
            ),
            ScoreEncoder(
                ["tasks_bucket_12", "content_id"],
                cv=cv,
                parent_prior="tasks_bucket_3_content_id_encoded",
                updatable=True,
                **self.params["score_encoder_2"]
            ),
            ...
            RollingScoreEncoder(
                ["user_id", "question_part"], count=True, time_since_last=True, **self.params["user_score_encoder"]
            ),
            RollingScoreEncoder(
                ["user_id", "question_category"], count=True, time_since_last=True, **self.params["user_score_encoder"]
            ),
            RollingScoreEncoder(
                ["user_id", "question_tag"], count=True, time_since_last=True, **self.params["user_score_encoder"]
            ),
            RollingScoreEncoder(
                ["user_id", "content_id"], count=True, time_since_last=True, **self.params["user_content_score_encoder"]
            ),
            ...
            RollingScoreEncoder(
                ["user_id", "question_part"], weighted=True, **self.params["user_weighted_score_encoder"]
            ),
            RollingScoreEncoder(
                ["user_id", "question_category"], weighted=True, **self.params["user_weighted_score_encoder"]
            ),
            RollingScoreEncoder(
                ["user_id", "question_tag"], weighted=True, **self.params["user_weighted_score_encoder"]
            ),
            ...
            DensityTransformer(
                [
                    "user_id_question_category_count",
                    "user_id_question_part_count",
                    "user_id_question_tag_count",
                    "user_id_content_id_count",
                ]
            ),
            ColumnsSelector(
                columns_to_drop=[
    ...
    def build_cv(self, X, train, valid):
        ...
        cv = [
            (Xtrain.iloc[cv_train].index.values, Xtrain.iloc[cv_valid].index.values, True) for cv_train, cv_valid in cv
        ]
    ...
    def fit_lgbm(self, X, y, X_val, y_val):
        ...
        self.models.append({"model": model, "best_score": best_score, "best_iteration": model.best_iteration})
    ...
    def refit_model(self, X, y):
        ...
        train_set = lgb.Dataset(X, y)
        model = lgb.train(self.params["lgbm_params"], train_set, num_boost_round=self.best_iteration, verbose_eval=-1)
    ...
    def fit_catboost(self, X, y, X_val, y_val):
        ...
        self.models.append({"model": model, "best_score": best_score, "best_iteration": best_iteration})
    ...
    def dumps_context(self):
        ...
        self.context_encoders = [
            e for _, e in (self.lectures_pipeline.steps + self.pipeline.steps) if hasattr(e, "context")
        ]
